...
class AllocationNet(nn.Module):
    """MLP mapping ordinal marginal preferences to allocation probabilities.

    Input: marginal_rank [B, A, m] int in {0,...,R-1}
        -> one-hot per (agent, item): [B, A*m*R]
    Output: softmax over K allocations (masked by IR+PE+Balanced).
    """
    ...
    def __init__(self, cfg: Config):
        ...
        R      = cfg.num_ranks
        in_dim = cfg.num_agents * cfg.num_items * R
